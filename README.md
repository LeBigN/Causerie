...
- `service-worker.js` — fonctionnement hors-ligne (cache `causerie-v7`)
...
- Menus déroulants : animateur (SCOTTO Nicolas, LAOUAR Hilel, Autre…), fonction (RA,
  Conducteur de travaux, Autre…) et entreprises (CRPS, TCPP, TERMICLIM,
...
Remplacer `index.html`, puis passer `CACHE_NAME` à `causerie-v8`
(v9, v10…) dans `service-worker.js`, et `APP_VERSION` dans `index.html`.
